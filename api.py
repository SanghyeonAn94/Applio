...
class ExtractRequest(BaseModel):
    ...
    gpu: str = Field(default="0", description="GPU IDs separated by '-' (e.g., '0-1-2'), or '-' for CPU")
    sample_rate: int = Field(default=48000, description="Sample rate")
    embedder_model: str = Field(default="contentvec", description="Speaker embedder model")
    embedder_model_custom: Optional[str] = Field(default=None, description="Path to custom embedder")
    include_mutes: int = Field(default=2, description="Number of silent files to include (0-10)")
...
class TrainRequest(BaseModel):
    ...
    save_every_weights: bool = Field(default=True, description="Save weights every epoch")
    total_epoch: int = Field(default=1000, description="Total training epochs (1-10000)")
    sample_rate: int = Field(default=48000, description="Sample rate")
    batch_size: int = Field(default=8, description="Batch size (1-50)")
    gpu: str = Field(default="0", description="GPU IDs separated by '-'")
    pretrained: bool = Field(default=True, description="Use pretrained models")
    custom_pretrained: bool = Field(default=False, description="Use custom pretrained models")
    g_pretrained_path: Optional[str] = Field(default=None, description="Path to custom generator pretrained")
    d_pretrained_path: Optional[str] = Field(default=None, description="Path to custom discriminator pretrained")
